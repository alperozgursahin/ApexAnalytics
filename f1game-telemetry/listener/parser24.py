...
class PacketMixin(object):
    ...
    # listener/parser.py dosyasının içindeki PacketMixin sınıfında...

    def _format_type(self, value):
        """A type helper to format values"""
        
        # YENİ EKLENEN KISIM: Değerin bir ctypes dizisi olup olmadığını kontrol et
        if isinstance(value, ctypes.Array):
            # Eğer bir dizi ise, her bir elemanını tek tek formatlayarak
            # normal bir Python listesi haline getir.
            return [self._format_type(item) for item in value]

        # Değişiklik yok, kodun geri kalanı aynı
...
EVENT_STRING_CODE_TO_FIELD = {
    b"SSTA": "m_fastest_lap",  # Session Started
    b"SEND": "m_fastest_lap",  # Session Ended
    b"FTLP": "m_fastest_lap",
    b"RTMT": "m_retirement",
    b"TMPT": "m_team_mate_in_pits",
    b"RCWN": "m_race_winner",
    b"PENA": "m_penalty",
    b"SPTP": "m_speed_trap",
    b"STLG": "m_start_lights",
    b"LGOT": "m_start_lights",
    b"DTSV": "m_drive_through_penalty_served",
    b"SGSV": "m_stop_go_penalty_served",
    b"FLBK": "m_flashback",
    b"BUTN": "m_buttons",
    b"OVTK": "m_overtake",
    b"SCAR": "m_satefy_car",
...
class PacketEventData(Packet):
    ...
    # Bu sınıfa özel to_dict metodu yazarak ana metodu eziyoruz (override).
    def to_dict(self):
        # Önce PacketMixin'deki orijinal to_dict'i çağırarak header gibi
        # standart alanları bir sözlüğe çeviriyoruz.
        base_dict = super().to_dict()
        
        # Olay kodunu [66, 85, 84, 78] gibi sayı listesinden b'BUTN' gibi bir byte string'e çeviriyoruz.
        event_code = bytes(base_dict["m_event_string_code"])
        
        # Eğer bu kodu sözlüğümüzde tanıyorsak
        if event_code in EVENT_STRING_CODE_TO_FIELD:
            # Doğru alan adını sözlükten buluyoruz (örn: "m_buttons")
            field_name = EVENT_STRING_CODE_TO_FIELD[event_code]
            
            # Dağınık event_details içinden sadece ilgili olanı alıyoruz.
            # Örnek: base_dict["m_event_details"]["m_buttons"]
            correct_details = base_dict["m_event_details"][field_name]
            
            # base_dict'teki dağınık "m_event_details" anahtarını silip
            # yerine kendi temiz formatımızda yeni bir tane ekliyoruz.
            base_dict["m_event_details"] = {
                "event_type": event_code.decode(), # b'BUTN' -> "BUTN"
                "details": correct_details
            }
        else:
             # Eğer tanımadığımız bir event kodu gelirse, bunu belirtelim.
             base_dict["m_event_details"] = {
                "event_type": event_code.decode(),
                "details": "Unknown Event Type"
            }

        # Temizlenmiş ve düzenlenmiş sözlüğü geri döndürüyoruz.
        return base_dict
